...
The project to update the website navigation finishes first and that team member publishes their changes to the Publish Branch: "Staging Website".  We are working on removing the product page, and before we can publish to the Staging Website, CloudCannon prompts us to [update from our Publish Branch](/documentation/articles/update-from-a-publish-branch/). When we try to update, CloudCannon detects the conflicting versions of the `/products/` file: on our Site we have changed a link, but on the Publish Branch the `/products/` page doesn't exist any more.

Before we can publish our changes, we need to tell CloudCannon which version of the file to keep: the changes on our current Site, the incoming changes from the Publish Branch, or a hybrid of the two. For more information, please read our documentation on [resolving publishing conflicts](/documentation/articles/resolve-publishing-conflicts/).
