...
<comp.Notice info_type="">
  <p>The following configuration is incorrect, as the <code>_structures</code> key is not the same as the <code>_inputs.*.options.structures</code> key:</p><comp.MultiCodeBlock language="YAML" translate_into={["json"]} source="cloudcannon.config.yml">
  ``````````
