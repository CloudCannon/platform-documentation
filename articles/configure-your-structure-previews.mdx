...
We want to create an array of staff members for our staff web page. The staff array can contain items which use either an "Employee" or a "Manager" template. We'll create these item template using two structures.

The Employee structure will have fields for a name, job description, and profile picture. The Manager structure will have the same fields but also include a URL field for a link to their calendar. We will configure these fields under the `value` key in each structure.
...
<comp.MultiCodeBlock language="YAMLs2" translate_into={["json"]} source="cloudcannon.config.yml">
```
_inputs:
  staff:
    type: array
