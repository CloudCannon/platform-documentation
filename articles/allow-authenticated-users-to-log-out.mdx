...
You can allow authenticated users to log out by adding a logout link to your authenticated routes. CloudCannon reserves the `/logout` route on all Sites. The logout URL for your Site will be your domain name followed by `/logout`, even if you serve your Site on a [subdomain](/documentation/articles/add-a-subdomain/), or [subpath](/documentation/articles/host-your-site-on-a-subpath/), CloudCannon w (e.g., `example.com/logout`, `app.example.com/logout`, or `example.com/documentation/logout`).
...
When a visitor clicks the logout link, CloudCannon will remove the cookie and reroute them to your home page.
...
If you use [authenticated routes](/documentation/articles/configure-authenticated-routes/), you may have a mix of authenticated and non-authenticated users on your public pages. In this case, you can use JavaScript and CSS to only show the logout button to authenticated users.
...
In the above example, the user's browser will add the `authenticated` class to the `body` element of your page if they have the `authenticated` cookie. The CSS file will only display elements with the `.logout` class if they are nested within an element with the `.authenticated` class.