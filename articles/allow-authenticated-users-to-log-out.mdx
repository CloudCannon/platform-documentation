...
You can allow authenticated users to log out by adding a logout link to your authenticated routes. CloudCannon reserves the `/logout` route on all Sites. The logout URL for your Site will be your domain name followed by `/logout`, even if you serve your Site on a [subdomain](/documentation/articles/add-a-custom-domain-to-your-site/#host-on-a-subdomain), or [subpath](/documentation/articles/add-a-custom-domain-to-your-site/#host-on-a-subpath), CloudCannon w (e.g., `example.com/logout`, `app.example.com/logout`, or `example.com/documentation/logout`).
