...
<comp.Notice info_type="info">
  The `default` schema is used if `_schema` does not exist. You can change the field from `_schema` with the `schema_key` option when [defining your collections](/documentation/articles/define-your-collections/).
</comp.Notice>
...
<comp.Notice info_type="info">
  You can control whether inputs are reordered, removed and/or hidden with the `reorder_inputs`, `remove_empty_inputs` and `hide_extra_inputs` [schema options](/documentation/articles/create-a-schema/#options).
</comp.Notice>
