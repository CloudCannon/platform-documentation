...
2. Snippet Picker
3. [Structures](/documentation/articles/create-a-structure/) inside [Array inputs](/documentation/articles/array-inputs/)
...
7. Non-structure items inside [Array inputs](/documentation/articles/array-inputs/)
8. [Collection](/documentation/articles/define-your-collections/) file lists
...
### Templating in previews

When using the `template` key in a preview's cascading options, you can reference keys from the object. If **any** placeholder resolves to an empty result, the whole preview entry is skipped, and the next preview entry is run.

See [this article for more information about using template strings](/documentation/articles/configuring-with-placeholders/), as well as a list of available filters.

Take the following config for example. The preview will show 1 line of metadata with the "person" icon. The text for this metadata will show "By Ella" if the object has an object `author` with a `name` field set to "Ella". If `author.name` isn't set, the text will show "No author".
