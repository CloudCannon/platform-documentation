...
The Object input provides a user interface for a group of inputs. Object inputs appear as [Cards](/documentation/articles/configure-your-card-previews/) in the Data Editor or sidebar of the Visual or Content Editor, previewing some of the content they contain.
...
You can configure an Object input using the `type` key with the value `object`, and the `subtype` key with the value `object`. Define the `type` key under your input key name within `_inputs` and the `subtype` key under `options`. For more information, please read our documentation on [inputs](/documentation/articles/what-are-inputs/).
...
You can configure a Mutable Object input using the `type` key with the value `object`, and the `subtype` key with the value `mutable`. Define the `type` key under your input key name within `_inputs` and the `subtype` key under `options`. For more information, please read our documentation on [inputs](/documentation/articles/what-are-inputs/).
