...
To create a file glob, type the file path of any files you want to determine access for. For more general file paths, you can create a glob using the special characters `*` and `{ }`. The `*` character denotes any file, while `**` denotes any folder. Using the `{ }` brackets, you can specify a list of options separated by a comma.
