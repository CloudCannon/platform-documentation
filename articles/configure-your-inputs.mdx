...
<comp.DataReference>
  ...
  <comp.DataReferenceRow label="hidden" type_markdown="Boolean or string">
    ...
    If set as a string, the input is hidden based on the value of another input. This can start with a `!` to reverse the value. For reverse values in `YAML` files, the string should be in `'` of `"` quotation marks.
