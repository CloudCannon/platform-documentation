...
<comp.DataReference>
 ...
 <comp.DataReferenceRow label="options.attributes_elements.inline" type_markdown="string">
  This key determines how to save Markdown format attributes for inline elements.
...
CloudCannon uses sensible defaults for `markdown.options.attributes_elements` based on your SSG. You should only override these if you're using an SSG that requires a different syntax for Markdown attributes than these defaults.
...
<comp.Conditional ssgs={["Hugo"]}>    
  If your `markdown.engine` is `commonmark` and you're using Hugo, `markdown.options.attributes_elements` will default to:
  ...
  CloudCannon gives different defaults for Hugo to account for differences in syntax between Goldmark and other CommonMark parsers.
</comp.Conditional>
<comp.Conditional not_ssgs={["Hugo"]}>    
  If your `markdown.engine` is `commonmark` (and you're not using Hugo), `markdown.options.attributes_elements` will default to:
