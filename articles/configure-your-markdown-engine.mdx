...
<comp.DataReference>
 ...
 <comp.DataReferenceRow label="options.quotes" type_markdown="string">
    ...
    **Supported for Kramdown only.**
    ...
    If `true`, headings within your Markdown will automatically be given ID attributes. Since `heading_ids` is a display-only option, this is only relevant when rendering in the Visual Editor if your site implements special behavior for headings with IDs (e.g. an on-hover style).
 </comp.DataReferenceRow>
</comp.DataReference>