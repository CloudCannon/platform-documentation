...
You can access nested keys and items within arrays. Let's assume the following data is being used:
...
Here are some examples of how this data could be accessed and manipulated. Note that when an array is indexed with `[*]`, each item will be joined with a comma and space.
