...
You can create a new collection file using the *\+ Add* button in the top right of the collection browser. The + Add dropdown shows a list of available Schemas for this collection, or if no Schemas are configured for this collection, it will generate a default template.

* Schema template - A Schema is a preconfigured file template that will populate new files with matching front matter and contents. You can configure your *\+ Add* to list a library of Schemas for each collection.
* Default template — The default template is generated if no Schemas are defined for a collection. It will populate new files with front matter (but not contents) matching the first file alphabetically in the collection. The default template will appear in the *\+ Add* dropdown as “+ Add \[collection item\]”, for example “+ Add Blog” for a collection named “Blog”.
...
3. Select the template or Schema you want to use.

CloudCannon will open a new file in an editing interface and save it in the location open in the collection or file browser.

## Upload from the file browser

The file browser allows you to browse the files on your site using the same file structure as your Git repository. You can upload an individual file or a folder of files. Uploading a folder uploads all files inside the folder, rather than the folder itself.
