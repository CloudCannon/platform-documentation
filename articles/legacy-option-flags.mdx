...
When you use a select input to reference a collection item, the value saved in your front matter is determined by [options.value\_key](/documentation/articles/what-are-select-and-multiselect-inputs/#value_key) configured for your input. If this option has not been configured, the value will fallback to look for one of several sensible keys to pull from, like "id" and "title".
