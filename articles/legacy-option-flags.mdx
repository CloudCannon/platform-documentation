...
When you use a select input to reference a collection item, the value saved in your front matter is determined by [options.value\_key](/documentation/articles/what-is-a-select-input/#value_key) configured for your input. If this option has not been configured, the value will fallback to look for one of several sensible keys to pull from, like "id" and "title".
...
`hide_link_to_telephone` is a configuration option that can be set on [URL inputs](/documentation/articles/configure-a-url-input) to hide the option to link to a telephone number. 
