...
Custom snippets are configured under the `_snippets` key, but provide a `snippet` string rather than a `template`. If we were wanting to configure a custom snippet that matched bash variables like `$HOME` or `$PATH`, we might use the following config:
