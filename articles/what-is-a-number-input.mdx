...
You can configure a Number input using the `type` key with the value `number`. Define the `type` key under your input key name within `_inputs`. For more information, please read our documentation on [inputs](/documentation/articles/what-are-inputs/).
...
<comp.CodeBlock language="Markdown" source="product-installation-guide.md">
```
...
You can configure a Range input using the `type` key with the value `range`. Define the `type` key under your input key name within `_inputs`. For more information, please read our documentation on [inputs](/documentation/articles/what-are-inputs/).
...
<comp.Notice info_type="info">
  When you load a  binary and hexadecimal number in the Data Editor,  will be converted to base-10 integers.
</comp.Notice>
