...
The Markdown rich text input stores all markup values as Markdown. There are two ways to configure an HTML rich text input: using the `type` key or the CloudCannon naming convention.
...
Using the key naming convention, CloudCannon will interpret any input key named `markdown` or that ends in `_markdown` as an HTML rich text input.
