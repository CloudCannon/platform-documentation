...
The entry input type specifies what type of input each new entry should be. Defining the entry input type for an Array input allows the Array to be empty without becoming misconfigured. All new entries added to an Array input will match the input type specified by the entry input type.
...
Normally, Array inputs are misconfigured when empty as CloudCannon doesn't know what to populate them with. Structured Arrays avoid this issue. In configuring a structured Array, you will create at least one predefined structure to populate it.

If you delete the contents of the Array input or create a new instance of that input, you can repopulate it with any of the structures you have configured.
...
Alternatively, you create the an Array called `values` to define your structures directly within your input configuration.
