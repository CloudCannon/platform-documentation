...
Custom redirects control what content is delivered to a visitor when they attempt to visit a URL. Redirects have three parts: the `from` URL, the `to` URL, and the `status` code.
