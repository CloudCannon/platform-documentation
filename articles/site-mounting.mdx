---
_schema: default
_uuid: a5baea74-78d6-4f50-939b-79e70a3218e5
_created_at: 2023-11-02T16:22:22+13:00
title: Site Mounting
nav_title: Site Mounting
published: true
image: /uploads/CloudCannonDocumentationog.jpg
description: Learn how to add Site Mounting with CloudCannon.
tags: []
related_articles: []
related_links: []
explicit_canonical:
---
Site Mounting makes one site dependent on one or more other CloudCannon sites. Files from a remote site will be accessible to the local site at build time, and any updates to the remote site will trigger a build of the local site.

* **Local Site**&nbsp;— The recipient of files from another site.
* **Remote Site(s)**&nbsp;— The donor site.

You can have several remote sites mounted to one local at a time. The number of connections per site and total number of mounted sites per Organization depends on your CloudCannon Plan.

<comp.Notice info_type="info">
  CloudCannon will prompt you to reduce your total site mountings before you can alter your pricing plan to one with a lower maximum limit.<!-- notionvc: ddcd9ad0-9196-4201-a664-d3ceabe8565c -->
</comp.Notice>

\[Why use site mounting\]

## Mount a new site

You can customize your settings on the&nbsp;*Site Mountings*&nbsp;page.

* **Remote artifact**&nbsp;— This determines which files are mounted and which events trigger new builds. You can set this to either *Source files* or *Build output* (available on <a target="_blank" rel="noopener" href="https://cloudcannon.com/pricing/">Team and Enterprise plans</a>).
* **Remote path**&nbsp;— This determines the path/folder mounted to this site from the remote site. Leave this blank to mount the entire site's files.
* **Local path**&nbsp;— This determines where CloudCannon will mount the remote files. This cannot be left blank.

To mount a site:

1. Open the local site.
2. Navigate to&nbsp;*Site Mountings*&nbsp;under&nbsp;*Site Settings*.
3. Select your&nbsp;*Remote Site*. This site must be within the same Organization as the Local Site.
4. Select your&nbsp;*Remote artifact*,&nbsp;*Remote path*, and&nbsp;*Local path*.
5. Click the&nbsp;*Mount site*&nbsp;button.
6. CloudCannon will add the site to your Sites mounted list.

<comp.DocsImage path="https://cc-dam.imgix.net/documentation/images/site-mounting/2023-Q4/CloudCannon-Documentation-Add-Site-Mounting.png" alt="A screenshot of the CloudCannon app shows the Site Mountings page, with a section to add a mounted site to the local site." title="Add a mounted site" type="screenshot"/>

## Remove a mounted site

To remove a mounted site:

1. Open the local site.
2. Navigate to&nbsp;*Site Mountings*&nbsp;under&nbsp;*Site Settings*.
3. Identify the mounted site you want to remove and click the&nbsp;*Context Menu*&nbsp;icon in the top right corner.
4. Click the&nbsp;*Unmount*&nbsp;button.
5. On the&nbsp;*Confirm unmount site*&nbsp;pop-up window, click&nbsp;*Unmount*. Click again to confirm.
6. CloudCannon will remove the site from your Sites mounted list.

<comp.DocsImage path="https://cc-dam.imgix.net/documentation/images/site-mounting/2023-Q4/CloudCannon-Documentation-Remove-Site-Mounting.png" alt="A screenshot of the CloudCannon app shows the Site Mountings page, with a context menu to unmount a site from the local site." title="Remove a mounted site" type="screenshot"/>