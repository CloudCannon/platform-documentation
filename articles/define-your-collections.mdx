...
<comp.Conditional ssgs={["Jekyll"]}>
  ...
  The automatic configuration is a good start, but we recommend adding collections and/or extending configuration for automatically discovered collections in your [global configuration](/documentation/articles/create-a-global-configuration-file/) under `collections_config`.
  ...
  <comp.Notice info_type="info">
    If required, you can prevent CloudCannon from automatically discovering collections by enabling the `collections_config_override` settings in your [global configuration](/documentation/articles/create-a-global-configuration-file/).
  </comp.Notice>
</comp.Conditional>
...
<comp.Conditional ssgs={["Hugo"]}>
  ...
  ### Including the collection index file

  Your collection's index file is named `_index.[extension]` and exists at the root of the collection's directory. It is usually used to list the files within that collection, and is sometimes referred to as a list page. For example, you might have a `/posts/` page which lists all your blog posts. This page would get its content from your `posts` collection's index file, i.e. `content/posts/_index.md`.

  **By default, CloudCannon does not apply collection configuration to the index file.** This is because index files are often structured differently to the rest of the collection. For example, your `posts` index file probably does not need front matter fields for `author` and `publish_date`.

  There may be some circumstances where you want to apply your collection configuration to  the index file. To do this, use the `parse_branch_index` option:
...
<comp.Conditional not_ssgs={["Jekyll", "Eleventy", "Hugo"]}>
  Define collections and configuration in your [global configuration](/documentation/articles/create-a-global-configuration-file/) under `collections_config`.
...
<comp.DataReference>
  ...
  <comp.DataReferenceRow label="preview" type_markdown="Object">
    ...
    You can also set `preview` for each [schema](/documentation/articles/create-a-schema/#options) in your collection.
  </comp.DataReferenceRow>
  ...
  <comp.DataReferenceRow label="schemas" type_markdown="Object">
    ...
    [Read more about schemas and the available options](/documentation/articles/create-a-schema/).
  </comp.DataReferenceRow>
  ...
  <comp.DataReferenceRow label="singular_key" type_markdown="String">
    Overrides the default singular input key of the collection. This is used for naming conventions for [select and multiselect inputs](/documentation/articles/select-and-multiselect-inputs/).
  </comp.DataReferenceRow>
