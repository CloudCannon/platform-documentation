...
<comp.Conditional ssgs={["Hugo"]}>
  ...
  Your collection's index file is named \_index.\[extension\] and exists in the base of your collection's directory. It is usually used to list the files within that collection, and is sometimes referred to as a list page. For example, you might have a /posts/ page which lists all your blog posts. This page would get its content from your post collection's index file.

  By default, CloudCannon does not apply your collection configuration to your index file. This is because index files are usually structured differently to the rest of your collection. For example, your posts index file probably does not need front matter fields for author and publication date.

  There may be some circumstances where you want to apply your collection configuration to  the index file. To do this, use the parse\_branch\_index option:
