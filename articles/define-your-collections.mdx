...
For sites that have not yet migrated to Unified Configuration, [view this page on our legacy documentation site](https://legacy.cloudcannon.com/documentation/articles/define-your-collections/).
</comp.Notice>
...
<comp.DataReference>
  ...
  <comp.DataReferenceRow label="glob" type_markdown="Array of Strings">
    Applies glob filtering to this collection, relative to the path.

    Globs can be positive (e.g. `*.yml`), or negative (e.g. `!**/*.json`).

    Files will be included in the collection if they match any positive glob, and do not match any negative glob.
    If no positive globs are provided, all files will be included unless they match a negative glob.
  ...
  <comp.DataReferenceRow label="include_developer_files" type_markdown="Boolean">
    By default, CloudCannon excludes files that probably shouldn't be edited in a CMS, for example files like `README.md` or `package.json`.
    These files will be excluded even if globs are configured.

    Setting `include_developer_files` to `true` will include these files in the collection, assuming they aren't filtered out by any configured globs.
  </comp.DataReferenceRow>
