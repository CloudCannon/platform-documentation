...
Before we can save any changes to other files or complete the sync to update all our files with incoming changes, we need to resolve the Syncing Conflict for `about.md` by saving or discarding our unsaved changes. In this example, we want to save our unsaved changes and let CloudCannon complete the sync.
...
Multiple Sites can contribute to the same Publish Branch. When someone publishes a change to your Publish Branch from another Site or branch, CloudCannon will notify you that an [update is available from your Publish Branch](/documentation/articles/update-from-a-publish-branch/). We recommend updating your Site from it's Publish Branch whenever updates are available.

A Publishing Conflict occurs when you have saved changes to a file on your Site which conflict with incoming changes to the same file.

To prevent one version of the file from overwriting the other, CloudCannon will stop attempting to update your files and alert you to the Publishing Conflict. CloudCannon will notify you about a Publishing Conflict using the *Conflicts* warning notification above the *Publish* button in your *Site Navigation*.
