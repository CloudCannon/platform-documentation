...
You can now choose the select values (separately from the naming convention) for a [select or multiselect input](/documentation/articles/configure-your-select-and-multiselect-inputs/) with `values`. Either define the values directly:
...
* [Color inputs](/documentation/articles/configure-your-color-inputs/) have new `format` and `alpha` options, falling back to the naming convention if these options are not set.
...
* Two new inputs: [Range number input](/documentation/articles/configure-your-number-inputs/#range) and a [Switch boolean input](/documentation/articles/what-is-a-boolean-input/#switch). These are only available with the new inputs config. More input types coming soon, contact support if you have specific requests.
