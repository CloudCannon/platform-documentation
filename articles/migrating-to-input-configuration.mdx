...
You can now choose the select values (separately from the naming convention) for a [select or multiselect input](/documentation/articles/what-are-select-and-multiselect-inputs/) with `values`. Either define the values directly:
