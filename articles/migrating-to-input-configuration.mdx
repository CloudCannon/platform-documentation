...
* Two new inputs: [Range number input](/documentation/articles/configure-your-number-inputs/#range) and a [Switch boolean input](/documentation/articles/configure-your-boolean-inputs/#switch). These are only available with the new inputs config. More input types coming soon, contact support if you have specific requests.
