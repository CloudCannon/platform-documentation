...
The `_inputs` configuration flips the input and configuration keys from the previous convention, allowing you to [define everything for an input in one place](/documentation/articles/how-to-choose-what-input-is-used-in-the-data-editor/).
...
You can now choose the select values (separately from the naming convention) for a [select or multiselect input](/documentation/articles/using-select-inputs-to-edit-your-data/) with `values`. Either define the values directly:
