...
<comp.Annotation number="3">
  This templae uses two placeholders and a filter. The key `commit_type` is defined below. The key `message` is undefined and will appear as a text input with the trim filter applied to the content.
</comp.Annotation>
<comp.Annotation number="4">
  `_inputs.commit_type` defines this key as a select input with three options, "feature", "fix", or "refactor".
</comp.Annotation>
</comp.MultiCodeBlock>
