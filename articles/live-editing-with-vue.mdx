...
* Components will need to use the Vue `onMounted` and `onBeforeUmount` hooks.
...
<comp.Annotation number="4">
  This needs to be run in the `onBeforeUmount()` hook to tear-down listeners before the component is destroyed.
</comp.Annotation>
</comp.CodeBlock>
