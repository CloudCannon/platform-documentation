...
CloudCannon uses the `localeCompare()` Javascript function when importing definitions for object keys in the main *Configuration File* and other *Configuration Files* referenced by a `*_from_glob` key. Any inline configuration from your main *Configuration File* is imported first, followed by configuration from other files in alphabetical [Unicode](https://en.wikipedia.org/wiki/List_of_Unicode_characters) order.
