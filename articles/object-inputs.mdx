...
Object inputs have two variants: the structured and the mutable Object input. With structured Objects, you can create sets of different keys to populate an Object, allowing your team to choose from several predefined [structures](/documentation/articles/create-a-structure/). With mutable Objects, your team can create their own key names and add, delete, clone, and reorder nested keys.
...
If your default Object input becomes empty, it is misconfigured.

There are two ways to tell CloudCannon how to populate your Object input, by configuring a mutable Object with an entry input type, or creating a structure. These Object input configurations are mutually exclusive and have different options available for configuration.

### Mutable Object

A mutable Object allows your team to add, delete, clone, rename, and reorder nested keys. Mutable Objects function similarly to [Array](/documantation/articles/array-inputs/) inputs, except that they do not allow for multiple nested keys with the same name.
...
By default, CloudCannon will copy the input type of most recent nested key in your Object input when you add a new nested key. This behavior can be beneficial for simple inputs, however it does not work if an Object is empty. To avoid this, CloudCannon will prevent you from deleting a nested key within an Object if it is the only nested key remaining.

For the best functionality, you should also define the entry input type for each mutable Object.

The entry input type specifies what type of input each new entry should be. Defining the entry input type for an Object input allows the Object to be empty without becoming misconfigured. All new nested keys added to an Object input will match the input type specified by the entry input type.
...
<comp.Notice info_type="info">
  CloudCannon allows your team members to name keys within mutable Objects anything they like, which can quickly lead to data inconsistency. You can address this issues by configuring the `entries` option for your mutable Object to define the allowed key names, reference [structures](/documentation/articles/create-a-structure/) to populate your keys, and assign specific structures to key names.
</comp.Notice>
...
<comp.Notice info_type="info">
  This input type relies on structures. For more information about how to configure structures, please read our [structures documentation](https://bubblegum-dugong.cloudvent.net/documentation/articles/create-a-structure/).
</comp.Notice>
...
Normally, Object inputs are misconfigured when empty as CloudCannon doesn't know what to populate them with. Structured Objects avoid this issue. In configuring a structured Object, you will create at least one predefined structure to populate it.

If you delete the contents of the Object input or create a new instance of that input, you can repopulate it with any of the structures you have configured.
...
<comp.Annotation number="3">
  The `structures` option determines what should populate the input. In this case, it is an object containing the `values` Array. For more information about how to configure structures, please read our [structures documentation](https://bubblegum-dugong.cloudvent.net/documentation/articles/create-a-structure/).
</comp.Annotation>
</comp.MultiCodeBlock>
