...
You can configure a tabbed Object input using the `type` key with the value `object`, and the `subtype` key with the value `object`. Define the `type` key under your input key name within `_inputs` and the `subtype` key under `options`. For more information, please read our documentation on [configuring your inputs](/documentation/articles/configure-your-inputs/).
...
The Tabbed Object input provides a user interface for a group of inputs. Tabbed objects look the same as a normal Object input on the top scope: a [Card](/documentation/articles/configure-your-card-previews/) previewing some of the content it contains.

When you click on the Object Card, it will open to show the first level of nested inputs within that Object as tabs. You can have an unlimited number of tabs at this level.

<comp.DocsImage path="https://cc-dam.imgix.net/documentation/images/editing/inputs/object/2024-10/CloudCannon-Documentation-Tabbed-Object-First-Tab.png" alt="A screenshot of a Tabbed Object input shows two tabs at the top, one for each nested object." title="A Tabbed Object input" type="ui-snippet"/>
...
In unconfigured Object inputs, CloudCannon will clone the last entry in the Object to add a new nested key. This method is not possible with an empty Object. You can configure a mutable Object with an entry input type or create a structure to avoid this issue and support empty Objects.
