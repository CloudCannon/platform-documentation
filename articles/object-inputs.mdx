...
A mutable Object allows your team to add, delete, clone, rename, and reorder nested keys. Mutable Objects function similarly to Array inputs, except that they do not allow for multiple nested keys with the same name.
...
```
<comp.Annotation number="1">
  This Object input is called `glossary`.
</comp.Annotation>
<comp.Annotation number="2">
  The value of the `type` key determines the input type. This is an `object` input.
</comp.Annotation>
...
You can configure the entry input type for your Object input under the `_inputs` key. To specify future entries within a particular Object, use the key name of the Object followed by the `.*` characters. Define the type of input for those entries with the `type` key.
...
By default, adding a new nested input to a mutable Object input will clone the last input within the object. Additionally, CloudCannon will allow your team members to name keys anything they like. These behaviors can be beneficial for simple inputs, however it can limit the complexity of your Object inputs and quickly lead to data inconsistency.

You can address these issues by configuring the `entries` option for your mutable Object to define the allowed key names, reference [structures](/documentation/articles/create-a-structure/) to populate your keys, and assign specific structures to key names.

### Structured Object

A structured Object allows your team to select a predefined template to populate the input. This input type relies on structures. For more information about how to configure structures, please read our [structures documentation](/documentation/articles/create-a-structure/).
...
Normally, Object inputs are misconfigured when empty as CloudCannon doesn't know what to populate them with. Structured Objects avoid this issue. In configuring a structure Object, you will create at least one predefined structure to populate it.
...
Alternatively, you create the Array input `values` to define your structures directly within your input configuration.
