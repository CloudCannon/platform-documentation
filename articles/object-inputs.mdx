...
The Tabbed Object input provides a user interface for a group of inputs. Tabbed Objects look the same as a default Object input on the top scope: a [Card](/documentation/articles/configure-your-card-previews/) previewing some of the content it contains.
...
You can configure a tabbed Object input using the `type` key with the value `object`, and the `subtype` key with the value `mutable`. Define the `type` key under your input key name within `_inputs` and the `subtype` key under `options`. For more information, please read our documentation on [configuring your inputs](/documentation/articles/configure-your-inputs/).
...
In unconfigured Object inputs, CloudCannon will clone the last entry in the Object to add a new nested key. This method is not possible with an empty Object. You can configure an entry input type for your Mutable Objects or create a Structure

mutable Object with an entry input type or create a structure to avoid this issue and support empty Objects.
