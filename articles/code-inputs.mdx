...
For each input, you can configure the height of the code area, tab size, theme color, gutter visibility, syntax highlighting, and null value. You can also use the [general configuration options](https://bubblegum-dugong.cloudvent.net/documentation/articles/configure-your-inputs/#general-configuration-options) available for all inputs.
...
Once configured, the Code input will appear in the Data Editor or sidebar of the Visual or Content Editor when you add it to a data file or the front matter of a markup file.
...
<comp.DataReference>
  ...
  <comp.DataReferenceRow label="show_gutter" type_markdown="Boolean">
    This key toggles the lines numbers and code-folding controls. Defaults to `true`.
  </comp.DataReferenceRow>
  <comp.DataReferenceRow label="syntax" type_markdown="String">
    ...
    Alternatively, you can configure `syntax` with the naming convention as well, `syntax` is assumed to be the section preceding the normalized `_code_block` suffix (e.g. `my_javascript_code_block` for `javascript`).
  </comp.DataReferenceRow>
  <comp.DataReferenceRow label="tab_size" type_markdown="Integer">
    This key defines how many spaces lines are auto indented by, and how many spaces a tab is shown as. Defaults to `2`.
  </comp.DataReferenceRow>
  ...
  <comp.DataReferenceRow label="empty_type" type_markdown="string">
    This key determines how CloudCannon handle an empty value. This key does not apply to existing empty values.
