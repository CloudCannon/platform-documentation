...
<comp.DataReference>
 ...
 <comp.DataReferenceRow label="paths.uploads_relative" type_markdown="Boolean">
  If this is set, files will be saved with a path relative to your upload directory.

  For example, if `path.uploads` is configured as `[base_path]/images` and you upload `image.jpg` to `/blog/my-post.md`, the path will be saved as `/blog/images/image.jpg`.
