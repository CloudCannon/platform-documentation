...
<comp.DataReference>
  <comp.DataReferenceRow label="allow_custom_markup" type_markdown="Boolean">
    This key enables you to edit custom markup in rich text editors and explicitly accept the risk of unintentionally deleting elements that cannot be recreated. By default, `allow_custom_markup` is `false` for inputs and `true` for editable regions.
  ...
  <comp.DataReferenceRow label="remove_custom_markup" type_markdown="Boolean">
    This key enables you to strip custom markup from content edited in a rich text editor, allowing you to clean files of unnecessary HTML content. By default, `remove_custom_markup` is `false` for inputs and editable regions.
