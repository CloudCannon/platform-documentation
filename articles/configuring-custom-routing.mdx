...
<comp.DataReference>
  ...
  <comp.DataReferenceRow label="status" type_markdown="Number">
    One of 200, 301, 302, 307, 308 or 410. Used to define the [HTTP status code](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status) to use on the route.

    301, 302, 307 and 308 codes trigger a redirect to a new location resulting in two requests. 200 and 410 change the URL that is served resulting in a single request.
  </comp.DataReferenceRow>
  <comp.DataReferenceRow label="forced" type_markdown="Boolean">
    ...
    200 and 410 status codes are always forced.
  </comp.DataReferenceRow>
