...
By default, all the objects in your DigitalOcean Spaces bucket are private, and cannot be viewed without authentication. You will need to make them public, so that they can be viewed on your site and within CloudCannon. You can configure the permissions on each object separately within DigitalOcean, but ideally we want to make all objects in the bucket public default.

DigitalOcean Spaces is built on top of Amazon's S3 service, so you can use the AWS CLI to make changes to your bucket policy. [See the AWS guide for installing the CLI here](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html).
