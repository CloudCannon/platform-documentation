...
In this second example, we want to create a Multiselect input populated with the files in our `posts` Collection as value options. More specifically, we only want files where the structured data key `author` has a value of "Heather". We can configure the following filter on our input values:
...
In this third example, we want to use data from a data file in multiple inputs. We have the following data file containing important information for our Site:
