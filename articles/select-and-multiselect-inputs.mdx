...
A filter allows you to limit the available values for a Select or Multiselect input to a a subset of possible values. Filters are available for any string value of `options.values` including fixed data sets defined under `_select_data`, Collections, or data files.

Filters use structured data keys to determine the correct subset of values. Let's walk through a few examples.

We want to create a Multiselect input populated with the files in our `posts` Collection as value options. More specifically, we only want files where the structured data key `author` has a value of "Heather". We can configure the following filter on our input values:
...
``````````
<comp.Annotation number="1">
  Available values for the Mutiselect input `heathers_posts` are any items in the `posts` Collection where the `author` structured data key is equal to "Heather".
</comp.Annotation>
</comp.MultiCodeBlock>
...
We have the following data file containing important information for our Site:
...
<comp.Notice info_type="important">
  ...
  Create a new site, or use the [Legacy Collections Value Key](/documentation/articles/legacy-option-flags/#legacy-collections-value-key) flag to update this for your site.
</comp.Notice>
