...
<comp.DataReference>
  ...
  <comp.DataReferenceRow label="_inputs.*.options.show_link_to_telephone" type_markdown="boolean">
    This key toggles whether the "Link to a phone number" option for this URL input is visible.

    Setting this key to `true` will show this option.

    By default, this key is false (i.e., the option is not visible).
  </comp.DataReferenceRow>
</comp.DataReference>
