...
<comp.DataReference>
  ...
  <comp.DataReferenceRow label="_snippets_imports_from_glob" type_markdown="Array of Strings">
    This key defines globs that filter which files CloudCannon should use for *Collection* configuration. Values in this array are relative to the root of your repository (i.e., `/`, not the value of `source`) and must end in the file extension `.cloudcannon.snippets_imports.yml`. You can use this key anywhere you would use the `_snippets_imports` key in the configuration cascade.
...
### One configuration is overwriting another

* Check which definitions are inline versus in *Configuration Files* referenced by your `*_from_glob` key, as CloudCannon will preferentially use the configuration defined outside of the main *Configuration File* (i.e., `cloudcannon.config.yml`).
* If you have defined the same key in multiple*Configuration Files* referenced by your `*_from_glob` key, check the names of those files as CloudCannon will preferentially use the configuration defined in files later in the [import order](/documentation/articles/why-split-your-configuration-file/#merging-behavior-for-object-keys).
