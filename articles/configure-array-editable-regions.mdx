...
6. Identify the
7. Add the `data-editable="array-item"` HTML attribute to the x. If no appropriate tags are available, wrap the templating in the `<editable-array-item>` web component.
8. If your array can contain array items with different structures, you must add the `data-id` HTML attribute to the same DOM element, or to the `<editable-array-item>` web component. The value of `data-id` should be...
9. Optional. If you also want to edit the contents of each array item in the Visual Editor, add [Text Editable Regions](/documentation/articles/configure-text-editable-regions/) or [Image Editable Regions](/documentation/articles/configure-image-editable-regions/) to the content inside each array item.
