...
* The *Summary* tab has three sections: the *Publish* section, *Changes* section, and *Commits* section.
  ...
  * The *Commits* section on this tab shows the two most recent commits made on this Site. It also has a *See more* link to the *Commits* tab.
...
* The R*emove Custom Domain* button will disconnect the domain from your Site, but will not remove it from your Organization. You must disconnect a domain from all Sites before you can remove it from your Organization.
...
* You can now generate an Initial Site Settings file in the `.cloudcannon` folder of your Git repository using the *Generate initial-site-settings.json* button in the *Recommendations* task list.

### Duplicating files

We have changed the behavior of the *Clone* file action in the *Context Menu* and renamed it to *Duplicate*.
...
* Conflicting updates have been renamed to Syncing conflicts as CloudCannon now reports multiple types of conflicts.
* We have removed the Syncing conflicts pop-up notification in the Site Navigation and replaced it with the Conflicts warning notification over the Save button. This appearance is now consistent with Publishing conflicts.
...
* Fixed an issue where CloudCannon would display the inputs for a new Authentication method incorrectly when you selected a new method on the Authentication page, but did not confirm your choice.
