...
<comp.DataReference>
  <comp.DataReferenceRow label="collections_config_from_glob" type_markdown="Array of Strings">
    This key defines globs which filter the files visible in the Collection browser for a given Collection. Values in this array are relative to the root of your repository (i.e., /, not the value of `source`) and must end in the file extension `.cloudcannon.collections.yml`.
