...
<comp.DataReference>
  ...
  <comp.DataReferenceRow label="_structures_from_glob" type_markdown="Array of Strings">
    ...
    <comp.Example>
      In this example, we have several *Structure Configuration Files* in the `.cloudcannon/structures/` folder. The value of the `_structures_from_glob` key tells CloudCannon to only use the `seo.cloudcannon.inputs.yml` and `blog-details.cloudcannon.inputs.yml` files in that folder.
