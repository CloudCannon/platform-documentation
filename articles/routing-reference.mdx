...
<comp.DataReference>
  ...
  <comp.DataReferenceRow label="forced" type_markdown="Boolean">
    This key toggles whether a redirect rule should take priority over the requested URL, even if the web serve successfully finds content served to the URL.
...
<comp.DataReference>
  <comp.DataReferenceRow label="headers" type_markdown="Array">
    This key defines any custom headers for URLs on CloudCannon hosted Sites. The `headers` array often has another `headers` array nested within.
