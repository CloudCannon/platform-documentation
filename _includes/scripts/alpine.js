...
Alpine.store('conditionals', {
    ...
    register(k) {
        ...
        const selected_url = new URLSearchParams(window?.location?.search?.toLocaleLowerCase?.() || '?').get(k.toLocaleLowerCase());
...
Alpine.magic('setNavMemory', () => {
    return () => {
        const navState = {
            ...
            opened: [...document.querySelectorAll("#t-docs-nav details")].map(d => d.hasAttribute("open")),
            time: Date.now(),
        };
...
Alpine.magic('getNavMemory', () => {
    return () => {
        ...
        [...document.querySelectorAll("#t-docs-nav details")].forEach((d, index) => {
