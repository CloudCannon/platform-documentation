...
const formatScopes = (scopes: string[], helpers: Helpers) =>
  scopes.map((s, i) => {
    if (i) {
      return (
        <>
          , <i>{helpers.unslug(s)}</i>
        </>
      );
    } else {
      return (
...
const formatDocs = (docs: [string, string][]) =>
  docs.map(([action, doc]) => (
    <li key={action}>
      <code>{action}</code> → {doc}
    </li>
  ));
...
export default function PermissionsTree(
  { comp, permissions }: PermissionsTreeProps,
  helpers: Helpers,
) {
