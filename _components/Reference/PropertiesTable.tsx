import { resolveRef, getDisplayName } from './helpers.ts';
...
function ObjectProperties({ entry, currentUrl, helpers, withIds, slugify }: PropertiesTableProps) {
        ...
        <>
            ...
            {properties.length > 0 ? (
                properties.map(([key, ref]) => (
                    <dd key={key} id={withIds && slugify ? `prop-${slugify(key)}` : undefined}>
                        <RefItem 
                            docRef={ref} 
            ...
            {additionalProps.length > 0 && (
                <>
                    <dt id={withIds ? "additional-properties" : undefined}>Additional properties:</dt>
                    {additionalProps.map((ref, i) => {
                        ...
                        return (
                            <dd 
                                key={resolved?.gid || i} 
                                id={withIds && slugify ? `addprop-${slugify(label)}` : undefined}
                            >
                                <RefItem 
                                    docRef={ref} 
                                    currentUrl={currentUrl} 
...
function ArrayItems({ entry, currentUrl, helpers, withIds, slugify }: PropertiesTableProps) {
...
function AnyOfTypes({ entry, currentUrl, helpers, withIds, slugify }: PropertiesTableProps) {
...
export default function PropertiesTable({ entry, currentUrl, helpers, withIds = false, slugify }: PropertiesTableProps) {
