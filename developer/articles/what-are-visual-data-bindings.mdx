...
Data bindings tell CloudCannon to add overlays to the Visual Editor. These overlays are absolutely positioned on the page. Once clicked, the overlay is selected and the controls become available. These controls are dependent on the type of data that is bound. Edit, for example, will always be available. Clicking off the selected area will unselect the current element.
