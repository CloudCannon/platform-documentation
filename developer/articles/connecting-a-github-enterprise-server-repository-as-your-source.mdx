...
1. Navigate to *Org Settings* &gt; *Files* &gt; *GitHub Enterprise Server*.
...
After configuring your server, you will see the *Authentication* tab. Click the *Authenticate* button and follow the prompts on your GitHub Enterprise Server instance to authorize CloudCannon.

### Server configuration

Once configured, the *Server* tab shows your current server details (endpoint, authorize URL, token URL, App ID, and Client ID). Use *Update Configuration* to re-validate the connection with your server.

### Syncing with a repository

To connect a GitHub Enterprise Server repository and start syncing files:

1. Click the *Sites* button in the *App Sidebar* to open the *Sites Browser*.
2. Click *Create a Site*.
3. Select *GitHub Enterprise Server repository* from the *File source* dropdown.
...
If you need to revoke CloudCannon's access to your GitHub Enterprise Server account:

1. Navigate to *Org Settings* &gt; *Files* &gt; *GitHub Enterprise Server* &gt; *Authentication* tab.
...
If you need to completely remove the GitHub Enterprise Server configuration from your *Organization*:

1. Navigate to *Org Settings* &gt; *Files* &gt; *GitHub Enterprise Server* &gt; *Danger Zone* tab.
2. You must unauthenticate first before the disconnect button becomes available.
3. Click *Disconnect GitHub Enterprise Server*.

Disconnecting destroys the server configuration entirely. You will need to re-enter all server details (endpoint, App ID, keys, etc.) to reconnect. Sites currently syncing through this server will fail on their next sync with a provider error until a new connection is established.