...
author_notes:
  ...
  ui: needs_check
  notes: |
    Auth gating for specific Site routes — Site-level Enterprise feature.
    The page pairs with `enable-password-authentication` and
    `enable-user-account-authentication` but doesn't carry an explicit
    pricing notice; readers without context may not know it's Enterprise-
    only. UI prose around the *Authentication* settings should be
    verified given how often the Site Settings area moves.
---
