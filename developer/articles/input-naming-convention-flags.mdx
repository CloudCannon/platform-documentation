...
The *Infer Select values from input key* flag controls whether CloudCannon uses the input naming convention to determine which Collections or *Dataset*s should populate a Select or Multiselect input.

This convention involved naming the input the singular noun of a Collection or *Dataset* name. For example, an unconfigured input named `author` will automatically become a Select input referencing a Collection named `authors`.
