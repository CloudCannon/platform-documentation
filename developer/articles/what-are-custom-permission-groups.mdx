...
A read or write action is associated with each file glob. Additionally, when you specify a file glob in a permission or an exception, this will override any `site:file:read` and `site:file:write` resources selected from the resource tree. Because specifying a file glob implicitly includes the resource for read or write, the `site:file` checkbox in the resource tree is enabled by default.

To create a file glob, type the file path of any files you want to determine access for. For more general file paths, you can create a glob using the special characters `*` and `{ }`. The `*` character denotes any file, `/**/` denotes any number of folders, and a glob ending in `/**` matches all files within a folder and all of its subfolders. Using the `{ }` brackets, you can specify a list of options separated by a comma.
