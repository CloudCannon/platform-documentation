...
Removing a permission or exception is checked the same way whenever it would widen access. Deleting an exception restores the access it was carving out, and deleting a file glob on a permission can expose a broader unrestricted permission, so CloudCannon requires you to hold the underlying resource at that scope before you can remove one. Deleting a plain permission only narrows access and is always allowed.
...
Scope also applies to the create action. For example, the resource `site:create` with a Project scope will allow members of the Permission Group to create new Sites within the chosen Project, but not elsewhere in your Organization. Likewise, an exception for `site:create` scoped to a Project prevents members from creating Sites in that Project without affecting the rest of your Organization.
