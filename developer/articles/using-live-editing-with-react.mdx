---
_schema: default
_uuid: c002f90f-7a89-43d9-b238-3955ddec4604
_created_at: '2021-09-15 23:13:40 +0000'
details:
  title: Visual data previews with React
  description: >-
    Live editing on CloudCannon with React. Edit Front Matter and see the
    changes show up live.
  image: >-
    https://cc-dam.imgix.net/documentation/images/default-thumbnail/2023-Q4/CloudCannon-Documentation-Thumbnail.png
  related_articles:
    - _type: developer_articles
      item: 46d2a5fd-615a-4d33-be84-9d6704bf8160
    - _type: developer_articles
      item: 4dc27408-06fe-403e-8bb3-a439805625ea
author_notes:
  docshots: Needs docshots
---
React is a great at receiving new data and rerendering components. To make this integration easier CloudCannon has developed an open source library for easy installation. <a href="https://github.com/CloudCannon/react-connector" target="_blank" rel="noopener noreferrer">@cloudcannon/react-connector</a> is a higher order component that listens to live editing events and passes the new props to your component.

## Installation
Cloudcannon React Connector is available on npm. To install run:

<comp.CodeBlock language="shell" source="Shell">
```
npm i @cloudcannon/react-connector
```
</comp.CodeBlock>

## Next.js Integration
Update the code in `pages/_app.jsx` from:

<comp.CodeBlock language="jsx" source="pages/_app.jsx">
```
export default function App({ Component, pageProps }) {
  return <Component {...pageProps} />
}
```
</comp.CodeBlock>

to:

<comp.CodeBlock language="jsx" source="pages/_app.jsx">
```
import { CloudCannonConnect } from '@cloudcannon/react-connector'
export default function App({ Component, pageProps }) {
  const AppComponent = CloudCannonConnect(Component);
  return <AppComponent {...pageProps}/>
}
```
</comp.CodeBlock>

This should make no difference to your development or production environment. Loading the site in CloudCannon's Visual Editor and changing front matter will push new changes directly into the page props.

Any issues, please contact support or <a href="https://github.com/CloudCannon/react-connector/issues" target="_blank" rel="noopener noreferrer">raise an issue on the GitHub repository</a>.
