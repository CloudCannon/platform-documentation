...
File locking prevents other *Team Members* from making changes to a file while your custom integration is working with it. Use `api.file()` to access the specific file you want to lock, then call `claimLock()` before making changes and `releaseLock()` when you are done.
...
<comp.Annotation number="3">
  Only make changes if the lock was granted. If another *Team Member* holds the lock, `readOnly` will be `true`.
</comp.Annotation>
<comp.Annotation number="4">
