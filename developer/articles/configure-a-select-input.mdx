...
<comp.DataReference>
  <comp.DataReferenceRow label="_inputs.*.options.allow_create" type_markdown="Boolean">
    This key determines whether you are allowed to enter a new text value that does not appear in the *Dataset*. CloudCannon does not add new text values to the *Dataset*. Defaults to `false`.
  </comp.DataReferenceRow>
  <comp.DataReferenceRow label="_inputs.*.options.allow_empty" type_markdown="Boolean">
    This key toggles the presence of an empty option alongside the options listed under `values`. Defaults to `true`. When `false`, CloudCannon will use the first value in the *Dataset* by default.
  ...
  <comp.DataReferenceRow label="_inputs.*.options.values" type_markdown="Array, object or string">
    ...
    Value can be an array of options, or a string referencing a fixed *Dataset*, Collection, or data file.
