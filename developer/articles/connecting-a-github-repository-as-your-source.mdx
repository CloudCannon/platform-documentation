---
_schema: default
_uuid: be14efab-623a-47cd-9114-bdd2a5853b23
_created_at: '2021-09-15 23:13:40 +0000'
details:
  title: Connecting a GitHub repository as your source
  description: Learn how to connect a GitHub repository to your site with CloudCannon.
  image: ye_olde_images/files-source-syncing-github-github-selected.png
  related_articles:
    - _type: developer_articles
      item: a01c2606-bd15-4d5e-877f-1018cb6e2f72
author_notes:
  docshots: Added!
  priority: P1
  last_reviewed: 2026-06-05
  audited_at_sha: b66d1d7e
  app_sha: 8544b3c1ef
  factual: ok
  persona: ok
  style: ok
  ui: ok
---

Connecting GitHub allows you to work on your websites locally and have the changes sync to CloudCannon. File changes made on CloudCannon are also synced back to GitHub.

To connect a GitHub repository and start syncing files, follow these instructions:

1. Go to *Site Settings* / *Syncing*
2. Select **GitHub repository**
3. Click **Authenticate**

<comp.DocShot docshot_key="CloudCannon-Documentation-Site-Settings-Syncing-Select-GitHub-No-Authorization" title="The Syncing page with GitHub selected" alt="The Site Settings Syncing page with GitHub repository selected from the provider dropdown and an Authenticate button." type="screenshot" />

This redirects you to GitHub. Enter your credentials to continue.

<comp.DocsImage path="assets/external_screenshots/files-source-syncing-github-authentication.png" alt="GitHub authentication screen" type="screenshot" />

Give CloudCannon access to your GitHub Account by clicking **Authorize CloudCannon**.

<comp.DocsImage path="assets/external_screenshots/files-source-syncing-github-authorisation.png" alt="GitHub connection screen" type="screenshot" />

You’ll be redirected back to CloudCannon and asked to install the GitHub App.

<comp.DocShot docshot_key="CloudCannon-Documentation-Site-Settings-Syncing-Select-GitHub-Repository-Dropdown" title="The Syncing page with the GitHub repository dropdown" alt="The Site Settings Syncing page with GitHub authenticated and a prompt to install the GitHub App to grant repository access." type="screenshot" />

Install the CloudCannon GitHub App on your personal account or on your organization. Select to grant access to *all repositories* or *only select repositories*.

<comp.DocsImage path="assets/external_screenshots/files-source-syncing-github-installing.png" alt="Install GitHub App" type="screenshot" />

You’ll be redirected back to CloudCannon  to pick a repository to connect.

<comp.DocShot docshot_key="CloudCannon-Documentation-Site-Settings-Syncing-Select-GitHub-Complete" title="The Syncing page with GitHub connected" alt="The Site Settings Syncing page with a GitHub repository selected from the repository dropdown, showing the Configure the GitHub App link." type="screenshot" />

If you can't find your repository from the list, click **Configure the GitHub App **to change your repository access for an existing installation. Or click *Install new GitHub App* to install on a different account.

<comp.DocShot docshot_key="CloudCannon-Documentation-Site-Settings-Syncing-Select-GitHub-Configure-App" title="The Installed GitHub App modal" alt="The Installed GitHub App modal showing existing installations with a Configure access button." type="screenshot" />

<comp.Notice info_type="important">
  Any existing files on your website will be replaced with the contents of the selected repository. Click **Backup and Sync** to continue, or exit the page to cancel the process.
</comp.Notice>

GitHub is now connected. Changes you push to the Git repository are pulled in by CloudCannon. Any changes made on CloudCannon are automatically committed and pushed.

## Migrating to GitHub App

By migrating to the GitHub App integration, you will be able to control the repositories to which you give CloudCannon access.

To migrate to GitHub App, follow these instructions:

1. Go to *Organization Settings &gt; Files &gt; GitHub*
2. Ensure you are logged into GitHub with the same GitHub account displayed above
3. Click **Migrate to GitHub app**
4. Follow the steps to **Authorize CloudCannon** and subsequently **Install** the GitHub App

Once migrated, a confirmation email will provide a link to revoke any old authentication in GitHub. Webhooks to CloudCannon on repositories can be deleted.

<comp.DocShot docshot_key="CloudCannon-Documentation-Org-GitHub-Migration-Pending" alt="A screenshot of the GitHub Organization Settings page shows a migration banner prompting the user to migrate to the GitHub App integration." title="The GitHub Organization Settings page with a migration banner" type="screenshot"/>
