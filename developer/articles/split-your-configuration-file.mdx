...
For a complete list of configuration keys available for the *CloudCannon Configuration File*, please read our [Configuration File reference](/documentation/developer-articles/configuration-file-reference/) documentation.
...
<comp.DataReference>
  ...
  <comp.DataReferenceRow label="values_from_glob" type_markdown="Array of Strings">
    This key defines globs that filter which files CloudCannon should use for *Structure* value configuration. Values in this array are relative to the root of your repository (i.e., `/`, not the value of `source`) and must end in the file extension `.cloudcannon.structure-value.yml` (note the singular form of "value"). You can use this key anywhere you would use the `_structures.*.values` key. Please see our documentation on [structures_from_glob](/documentation/developer-articles/configuration-file-reference/#_structures_from_glob) for defining a *Structure* in a split *Configuration File*.
