---
_schema: default
_uuid: 4b5c6d7e-8f9a-0b1c-2d3e-4f5a6b7c8d9e
_created_at: 2023-11-02T03:22:22.000Z
details:
  title: Mount a Site
  description: Learn how to add Site Mounting with CloudCannon.
  image: /uploads/CloudCannonDocumentationog.jpg
  category:
    - Instructions
  related_articles:
    - _type: developer_articles
      item: a5baea74-78d6-4f50-939b-79e70a3218e5
    - _type: developer_articles
      item: 7c8d9e0f-1a2b-3c4d-5e6f-7a8b9c0d1e2f
author_notes:
  docshots: null
---
You can customize your settings on the *Site Mountings* page.

To mount a site:

1. Open the local site.
2. Navigate to *Site Mountings* under *Site Settings*.
3. Select your *Remote Site*. This site must be within the same Organization as the Local Site.
4. Select your *Remote artifact*, *Remote path*, and *Local path*.
5. Click the *Mount site* button.
6. CloudCannon will add the site to your *Sites mounted* list.

<comp.DocShot docshot_key="CloudCannon-Documentation-Site-Settings-Site-Mountings-Page" alt="The Site Mountings page shows an empty mounted sites list and a form to mount a new site with fields for Remote site, Remote artifact, Remote path, and Local path." title="The Site Mountings page" type="screenshot" />