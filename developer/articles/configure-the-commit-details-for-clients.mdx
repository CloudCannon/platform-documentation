---
_schema: default
_uuid: b81a259b-3cc9-406d-98df-d1e4ae58343c
_created_at: 2024-04-30T03:54:44.000Z
details:
  title: Configure the commit details for Clients
  description: Learn how to configure the commit details for a Client.
  image: >-
    https://cc-dam.imgix.net/documentation/images/default-thumbnail/2023-Q4/CloudCannon-Documentation-Thumbnail.png
  category:
    - Instructions
  related_articles:
    - _type: developer_articles
      item: 89a9acc9-dc94-4f5b-aea7-ba73c1534563
    - _type: developer_articles
      item: 23d6026f-031d-49d1-850e-95ec8e997e02
author_notes:
  docshots: Added!
  priority: P1
  last_reviewed: 2026-06-05
  audited_at_sha: b66d1d7e
  app_sha: 8544b3c1ef
  factual: ok
  persona: ok
  style: ok
  ui: ok
---
By default, when a Client edits your Site, the commit details will contain the name “CloudCannon” and email address “support@cloudcannon.com”.

You can configure the commit details for Clients to create a more accurate history of changes made to your Site.

To configure the commit details for a Client:

1. Navigate to the *Client Sharing* page under *Site settings*.
2. Click on the *Author* tab.
3. Under the *Author* tab, enter the name and email address you want to appear in the commit history.
4. Click the *Update* button.

<comp.DocShot docshot_key="CloudCannon-Documentation-Client-Sharing-Author-Tab" title="The Author tab on the Client Sharing page" alt="A screenshot of the Author tab on the Client Sharing page shows two fields for the name and email address applied to commits made by Clients." type="screenshot" />