...
A "404 page" is the webpage shown to visitors when the web server cannot find the content for the URL they requested, named after the <a href="https://en.wikipedia.org/wiki/List_of_HTTP_status_codes" target="_blank" rel="noopener noreferrer">HTTP status code</a> `404 Not Found`. A web server may be unable to find a page's content because:
...
<comp.DocsImage path="assets/onboarding_screenshots/CloudCannon-Documentation-404-Page.png" alt="A screenshot of CloudCannon's default 404 page." title="CloudCannon's Default 404 Page" type="screenshot"/>
