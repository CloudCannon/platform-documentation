...
<comp.DataReference>
  <comp.DataReferenceRow label="_inputs.*.options.timezone" type_markdown="String">
    This key determines which timezone to use when displaying and editing dates. This will also change the suffix of the date value. Defaults to the `timezone` configured in your [configuration file](/documentation/developer-articles/create-a-global-configuration-file/), which defaults to `Etc/UTC`.
