...
author_notes:
  ...
  ui: ok
  notes: |
    Code example around line 60 has a path inconsistency — the `pages_fr`
    collection shows `content/en` instead of `content/fr`. Fix the path so
    the example matches its label.
---
