...
Structures are CloudCannon's answer to structured data on static sites. You can read more about structures in [Create a structure](/documentation/developer-articles/create-a-structure/) and the [Structures reference](/documentation/developer-articles/structures-reference/), but the main thing to know is that for Bookshop, Structures define the set of components that can be added to a specific key in CloudCannon.
...
<comp.Annotation number="1">
  ...
  The `_inputs` section is passed through as [CloudCannon's input configuration](/documentation/user-articles/what-are-inputs/),
