...
author_notes:
  ...
  ui: needs_check
  notes: |
    References the "Command Line Options" menu in the Configuration section
    when explaining where to set the `TRANSLATE` environment variable.
    Verify the current UI path — the build-options view has shifted around
    in recent Lit conversions.
---
