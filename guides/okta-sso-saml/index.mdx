...
3. The user's Internet browser redirects them to the Single Sign-On (SSO) URL. In this case, the SSO URL would be an Okta Login page.
4. The Identity Provider (Okta) parses CloudCannon's SAML request. Okta authenticates the user by asking them to enter their Okta username and password. If the user is already logged in to Okta, then it skips this step.
5. The Identity Provider (Okta) generates a SAML response.
6. The user's Internet browser redirects them to CloudCannon, where CloudCannon validates the SAML response.
7. If the verification is successful, CloudCannon logs the user in and gives them access to your Organization.

When you configure SSO/SAML between an Identity Provider and a Service Provider
