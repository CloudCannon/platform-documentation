...
They main point of confusion when setting up SSO/SAML between an Identity Provider (i.e., Okta) and a Service Provider (i.e., CloudCannon) is the conflicting terminology used by different software, and the back-and-forth

&nbsp;

* **Single sign-on** **(SSO) URL** — The CloudCannon address to which Okta will redirect users after they enter their login in details. CloudCannon calls this the *SSO URL*.
* **SAML 2.0 Endpoint (HTTP)** — The Okta URL to which CloudCannon will redirect users when they try to log in to CloudCannon.

&nbsp;

* **Entity ID** — The intended recipient for your SAML assertion. This is also known as the Service Provider (SP). CloudCannon calls this the *Entity ID (Issuer)*.
...
The Single Sign-On process uses the following steps:
...
* **Entity ID** — The intended recipient for your SAML assertion, also known as the Service Provider (SP). CloudCannon calls this the *Entity ID (Issuer)*.
