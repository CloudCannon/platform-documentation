...
To render this array of components on the page, we can loop over them and use the dynamic components we covered in [Using Bookshop components](/documentation/guides/bookshop-astro-guide/using-components/#rendering-dynamic-components)\:
...
By passing all of the data to our `page` helper and looping within, Bookshop is able to update the entire page when the `content_blocks` array is changed.
