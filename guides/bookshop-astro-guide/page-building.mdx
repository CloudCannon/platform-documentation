...
as the `frontmatter.content_blocks.map` loop is not being rerendered.

The solution to this is to simply move this loop inside a Bookshop component. In this case,
...
Once you're done you should have a directory structure like this:
<comp.Tree>
```
src/
>> components/
...
Note that we don't need to add the `bookshop:live` directive to the subcomponents in a page
