---
_schema: guide-item
_uuid: 2acd2689-3137-4bd0-8c1c-a77db6cbdea9
_created_at: 2023-06-07T00:00:00Z
title: Introduction
nav_title: Introduction
published: true
order: 1
image: "/documentation/static/CloudCannonDocumentationog.jpg"
description: Discover our recommended best practices for integrating multilingual Hugo sites.
tags: []
related_articles: []
related_links: null
guide_listing:
  show_on_index: true
  category: CloudCannon Guides
---
Hugo has two approaches to manage translated content with files, by including the language code in the **filename** or **content directory/folder**.

The approach you choose is your preference. It does change the editing experience as CloudCannon collections are based around folders. We'll outline the best practices for each approach below.

Hugo does not support using both approaches at the same time, so only choose one.

<comp.Notice info_type="info">
This guide focuses on integrating with CloudCannon, it does **not** cover:

- Setting up a [multilingual Hugo](https://gohugo.io/content-management/multilingual/) site, we recommend [Hugo Multilingual Part 1: Content translation](https://www.regisphilibert.com/blog/2018/08/hugo-multilingual-part-1-managing-content-translation/) by Régis Philibert.
- The Hugo `i18n` shortcode, which could be added as a [snippet](/documentation/articles/editing-with-hugo-shortcodes/).
- [Multihost](https://gohugo.io/content-management/multilingual/#configure-multilingual-multihost) introduces editing problems as a site on CloudCannon uses one domain. Using multiple sites in a staging-production workflow can work around this.
</comp.Notice>
