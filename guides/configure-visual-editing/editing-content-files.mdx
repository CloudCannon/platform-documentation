...
* `<text-editable>` — This web component allows you to wrap other elements and apply Editable attributes. This is useful if you want to edit an element that can't take attributes or a specific section of a string.
...
If we save and rebuild our Site, yellow editable boxes will appear around the title and author fields in the Visual Editor, enabling inline editing of those values.
...
Editing the body of a content file in the Visual Editor is the most intuitive editing experience for CloudCannon users.

In the blog layout, the `<slot />` element is populated by the Markdown content in the main body of each blog file. However, because `<slot />` is not a DOM element, we can't add the `data-editable=""` and `data-prop=""` attributes directly. To get around this issue, we can specifically wrap the `<slot />` element in a `<text-editable>` web component.
...
If we save and rebuild our Site, a yellow editable box will appear around the blog content in the Visual Editor, enabling inline editing.
