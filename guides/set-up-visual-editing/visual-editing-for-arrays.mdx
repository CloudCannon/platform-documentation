...
nav_title: Visual Editing for arrays
order: 8
published: true
image: >-
...
There are no obvious DOM elements we can use to define the Array Item *Editable Region*. In this case, we'll wrap each item in the `<editable-array-item>` web component. Because this is a complex array, we must also add the `data-id` HTML attribute with a templating value, such as `{block._name}`. This field will populate with the unique identifier for each array item at build time.
