...
Next, as the `<CTA>` component has no appropriate parent DOM element, we should wrap it in the `<editable-component>` web component.

Our Component *Editable Region* needs a `data-prop` to define the path the to data property we want to edit, and a `data-component` to define which registered component we are using. Let's add the HTML attribute `data-prop="cta"`, as `cta` is the key for our object in `first-post.mdx`, and `data-component="cta"`, as `cta` is also the name of the component we registered.
