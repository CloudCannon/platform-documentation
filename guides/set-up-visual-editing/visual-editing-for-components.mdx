...
Many static website use reusable code components (a.k.a., partials) ranging from a small snippet of HTML, to UI elements like a header, profile card, or an entire page layout.

Components use templating to reference structured data keys. Layout files then reference a component file. At build time, CloudCannon populates the component with the correct values and outputs it to the layout's HTML webpage. You can use the Component *Editable Regions* to re-render changes to your components in the Visual Editor, or enable inline editing of component text and images in conjunction with other *Editable Regions*. Let's cover how to define one.
...
First you need to import the component registration file you made earlier. Inside the `<body>` element of our layout, you can add a `<script>` HTML element with instructions to import the component registration file if the window is in Editor Mode (i.e., your webpage is open in CloudCannon's Visual Editor).
