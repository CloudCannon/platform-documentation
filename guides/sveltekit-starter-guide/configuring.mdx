...
SvelteKit takes a less opinionated approach and allows you to decide where and how content is stored.
This means that for SvelteKit, CloudCannon out of the box only has the source editor available.
...
To get the most out of CloudCannon, you will need to [run SvelteKit as a Static Site Generator](https://kit.svelte.dev/docs/adapter-static).
You can do this by using SvelteKit's `adapter-static` adapter.

First, install the adapter by running `npm install -D @sveltejs/adapter-static`.<br/>
Then, in your `svelte.config.js`, import the static adapter and set it as the `adapter` in the `kit` config object, like so:

<comp.CodeBlock language="javascript" source="svelte.config.js">
```
...
You now may need to convert your `+page.js` and/or `+layout.js` files to `+page.server.js` and `+layout.server.js` files, respectvely.
...
We recommend storing your page content in files using one the file formats listed above.
...
CloudCannon has an open-source package called [Filer](https://github.com/cloudcannon/filer) which lets you easily pull data from your content files and collections.

For example, in a `+page.server.js` (or equivalent) you could fetch your `content/about.md` file data like so:

<comp.CodeBlock language="javascript" source="+page.server.js">
```
import Filer from '@cloudcannon/filer';

export async function load() {
  const contentFile = 'about.md';
