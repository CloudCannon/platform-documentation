...
- Rosey is open-source, so if there’s something you don’t like, you can create an issue or GitHub or even submit a pull request to have it changed. The code is completely public if you want to check out what’s happening under the hood.
...
- Rosey has a much wider feature set
...
One problem with using Rosey with CloudCannon is that it can interfere with the Visual Editor. The Visual Editor needs to know where your site's output files are, and in the workflow described in this guide Rosey moves those files around. For this reason, we recommend using Rosey **only** in conjunction with a staging workflow.

You can read more about the advantages of a staging workflow, and how to set it up in CloudCannon, [in this Staging Workflow Guide](/documentation/guides/staging-workflow-guide/).
