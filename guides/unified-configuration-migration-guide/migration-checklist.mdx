...
I forgot to mention, but the build integrations also would discover collections (similar to how gadget does) at build time. This will no longer happen - so they'll also need to add any collections\_config entries here that were auto-discovered (e.g. posts/pages/drafts in Jekyll).<br>Effectively, it's like we have the now defunct `collections_config_override` force set to "true" for all decoupled sites.
...
2. `collections_config.*.output_url`
...
For example, `value_key: path` and `value_key: url` will continue to work, however Hugo's `value_key: content_path` and Jekyll's `value_key: id` will not work as CloudCannon generates these values during your last successful build.

### Explicitly configure attributes for Editable Regions

CloudCannon no longer automatically applies Editable Regions to data that uses file content without an explicit attribute. Ensure that all Editable regions include the `data-cms-edit="content"` attribute.

The below code shows how to configure the `data-cms-edit="content"` attribute for your Editable Regions:
