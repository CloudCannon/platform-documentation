...
1. `data_config.*.parser` (Note: Please see the above step regarding data set formatting.)
...
CloudCannon now uses a different method to determine which files are visible within a collection. Please replace the `collections_config.*.filter` key with a file glob. A file glob is a pattern used to identify matching files. In your collections, the `collections_config.*.glob` key is an Array. Each entry in the array is a pattern that controls which files are visible in CloudCannon's collection browser. The glob pattern is relative to your collection path, and can include negative values by adding the `!` character as a prefix.
...
Additionally, to ensure that what you see in your collection browser is what is assigned to that collection, all collections now use `filter.base: strict` by default.
