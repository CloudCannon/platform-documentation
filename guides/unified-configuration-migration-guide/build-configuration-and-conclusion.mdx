...
Previously, the output URL for a file was read from your SSG at build time. Instead, CloudCannon now determines output URLs using heuristics after building your Site.

Most Sites will be unchanged, however some Sites may fail to auto-detect the correct URLs. We recommend checking that the URLs displayed in CloudCannon are correct.

URLs are used when copying the output path of a collection item, and when opening the Visual Editor for a file. If you experience any issues with these actions, we recommend [configuring explicit URLs for your collections](/documentation/articles/define-your-collections/#url).
