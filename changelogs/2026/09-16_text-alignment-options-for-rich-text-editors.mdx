...
* Added the `align` option to *Rich Text Editors*, allowing you to align blocks of text without
  configuring a class name for each alignment. CloudCannon saves alignment as a `text-align` style by default.
...
* Fixed an issue where filtering assets in some DAMs returned every asset, rather than only the assets matching your search term.
...
* Fixed an issue where updating or deleting a *Site* served from a subpath did not clear the cache for the *Site* above it.
* Fixed an issue where `file_config` entries ignored `_inputs_from_glob`, `_structures_from_glob`, and other `_from_glob` keys.
* Fixed an issue where scrolling the *Inboxes* dropdown on the *Forms* page would duplicate the *New inbox* option.
