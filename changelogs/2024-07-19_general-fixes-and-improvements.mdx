...
type: minor
---
This release addressed several issues, including load times and UX when uploading files in the DAM browser, signaling the maximum length of custom domain names, and backups failing for very large sites.
...
* Options for removing a user from an Client Organization are now correctly hidden if that user is a Partner.