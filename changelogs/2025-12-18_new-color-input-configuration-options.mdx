...
* You can now configure the `palette` option for *Color Inputs*. This key provides a palette below the color picker for important or recently used colors. When you close the color picker, CloudCannon will add the current color value in the color spectrum to the palette.
* You can now configure the `togglePaletteOnly` option for *Color Inputs* with the `palette` option configured. This key toggles whether CloudCannon displays the color spectrum picker and the palette. When set to `true`, the *Color Input* will only display the palette.
