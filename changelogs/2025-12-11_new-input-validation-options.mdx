...
* For Array Editable Regions, you can now use `<template>` HTML elements to populate new array elements rather than registering components. You can add the `data-id` attribute to each `<template>` element, allowing you to define several array item templates.
