...
* Added the `allow_custom_markup` key to enable the capacity to edit any HTML or Markdown structure that cannot be recreated in the rich text editor.
...
* Added the `remove_custom_markup` key to strip HTML that cannot be recreated in the rich text editor.
  * This allows you to clean files of unnecessary HTML content.
  * This key is disabled if `allow_custom_markup` is enabled.
  * You can define the `remove_custom_markup` key anywhere you would configure a rich text toolbar. By default, this key is `false` for inputs and editable regions.
  * You can also enable `remove_custom_markup` in the configuration GUI.
