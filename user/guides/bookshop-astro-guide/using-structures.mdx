...
Structures are CloudCannon's answer to structured data on static sites. You can read more about structures in [Create a structure](/documentation/developer-articles/create-a-structure/) and the [Structures reference](/documentation/articles/structures-reference/). The main thing to know is that for Bookshop, Structures define the set of components that you can add to a specific key in CloudCannon.
