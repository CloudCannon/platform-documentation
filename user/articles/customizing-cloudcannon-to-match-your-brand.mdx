---
_schema: default
_uuid: 0c2483de-312f-44c1-a2e4-ef00e4679d48
_created_at: '2021-09-15 23:13:40 +0000'
details:
  title: Customizing CloudCannon to match your brand
  description: Learn how to customize CloudCannon to match your brand.
  image: /documentation/static/CloudCannonDocumentationog.jpg
  related_articles:
    - _type: user_articles
      item: 35f4fd87-5396-4316-85ca-24443d98e307
    - _type: user_articles
      item: 878bd1e4-b0e0-411e-8e26-b5b4a68f8969
author_notes:
  docshots: Added!
  priority: P1
  last_reviewed: 2026-06-05
  audited_at_sha: b66d1d7e
  app_sha: 8544b3c1ef
  factual: ok
  persona: ok
  style: needs_update
  ui: ok
  notes: |
    Step phrasing drifts between "Go to" and "Navigate to"; button labels
    like "Update Organization" appear unitalicised in places. Align with
    the style guide's voice and UI-element formatting conventions.
---

Bring your brand to CloudCannon.

You can add your brand to CloudCannon in three major ways:

1. Set a Brand color
2. Organization Badge
3. Organization Sites List Logo

## Brand Color
Brand color defines the color of the sidebar and many other UI elements in CloudCannon.

To set your brand color:

1. Go to *Organization Settings* / *Details*
2. Use the color picker to set a color
3. Click **Update Organization** to confirm the configuration

## Add your brand assets
To add the assets defined above:

1. Go to *Organization Settings* / *Details*
2. Upload an image for both available assets
3. Click **Update Organization** to confirm the configuration

<comp.DocShot docshot_key="CloudCannon-Documentation-Org-Branding-Settings" title="The Organization branding settings" alt="Screenshot of organization branding interface" type="screenshot" />

## Organization Badge
This asset is used in the organization list. For best results, the image should be square with a minimum width of 256px.

<comp.DocShot docshot_key="CloudCannon-Documentation-Account-Organizations-Click-Through-List" title="The Organizations list" alt="Screenshot of Organizations list" type="screenshot" />

## Organization Sites List Logo
This asset is used in the organizations site list. For best results, the image should be wider than it is tall and have a minimum width of 512px.

<comp.DocShot docshot_key="CloudCannon-Documentation-Org-Branded-Sites-List" title="The Sites list with custom branding" alt="Screenshot of Sites list with custom branding" type="screenshot" />
