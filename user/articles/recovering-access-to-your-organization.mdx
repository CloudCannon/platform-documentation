...
author_notes:
  ...
  ui: ok
  notes: |
    Compliance/account-recovery topic — the support-escalation path is
    exactly what *Compliance Officer* readers verify before signing off
    on CloudCannon. Page is informal ("we will sort something out",
    "be respectful") and lacks: the exact information support needs
    (account email, invoice ID, DNS verification flow), how long manual
    transfer takes, and what to do if the original Owner is unreachable.
    Worth promoting to P0/P1 and rewriting against the
