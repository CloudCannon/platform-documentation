...
<comp.Slot name="Schema-configuration">
  ...
  Schemas help you keep files consistent within a Collection. When you create a new Collection file, CloudCannon will automatically populate the front matter of the file with your inputs and structured data. CloudCannon will also update the front matter to match your Schema when you open an existing file.

  Here's an example of a Markdown schema. In this schema, we have four structured data keys in the front matter: `title`, `date`, `author`, and `image`. We want our team members to use this schema when creating new blog posts for our website.
